...
function getDraftVideoOfUser($uid){
	global $con;
	$execQry=mysqli_num_rows(mysqli_query($con,"select * from `tbl_add_videos` where video_status='0' and user_id='$uid' "));
	return $execQry;
}
function getCompleteVideoOfUser($uid){
	global $con;
	$execQry=mysqli_num_rows(mysqli_query($con,"select * from `tbl_add_videos` where video_status='1' and user_id='$uid' "));
	return $execQry;
}
function getPriceNameById($id){
	global $con;
	$execQry=mysqli_fetch_row(mysqli_query($con,"select `name` from `pricing` where `id`='$id'  "));
	return $execQry[0];
}
function getVideoOfUser($uid){
	global $con;$total=0;$draft=0;$complete=0;
	$execQry=mysqli_query($con,"select * from `tbl_add_videos` where user_id='$uid' ");
	$total=mysqli_num_rows($execQry);
	if($total>0){
	while($fetch=mysqli_fetch_array($execQry)){
		if($fetch['video_status']=='1') {
			$complete++;
		}else{
			$draft++;
		}
...
function getSubscriptionIdByUserId($uid){
	global $con;
	$qry=mysqli_fetch_row(mysqli_query($con,"select `price_id` from `users` where `id`='$uid'"));
	return $qry[0];
}
...
function getAdminEmail($id){
	global $con;
	$sqlQry=mysqli_query($con,"select `email` from `users` where `id`='$id'");
	$fetchQry=mysqli_fetch_row($sqlQry);
	return $fetchQry[0];
}
function getUserName($id){
	global $con;
	$sqlQry=mysqli_query($con,"select `name` from `users` where `id`='$id'");
	$fetchQry=mysqli_fetch_row($sqlQry);
	return $fetchQry[0];
}
...
function getUserImage($id){
	global $con;
	$sqlQry=mysqli_query($con,"select `profile_photo_path` from `users` where `id`='$id'");
	$fetchQry=mysqli_fetch_row($sqlQry);
	return $fetchQry[0];
}
function getSubscriptionDetailsById($id){
	global $con;
	$sqlQry=mysqli_query($con,"select * from `pricing` where `id`='$id'");
	$fetchQry=mysqli_fetch_row($sqlQry);
	return $fetchQry;
}
